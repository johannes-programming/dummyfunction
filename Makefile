...
test:
	conda run -n base python make/env.py test_dummyfunction --python=3.11 --recreate >/dev/null;
	conda run -n test_dummyfunction pip install -e . >/dev/null;
	conda run -n test_dummyfunction python -c 'import dummyfunction.tests; dummyfunction.tests.test();';
	conda run -n test_dummyfunction pip install mypy >/dev/null;
	conda run -n test_dummyfunction python -m mypy --strict .;
	conda run -n test_dummyfunction python -m mypy --strict -p dummyfunction;
